...
@section('content')
    ...
    <div class="container-fluid py-4">
        <div class="row">
            <div class="col-md-8">
                <div class="card">
                    <form role="form" method="POST" action={{ route('stages.affecterEncadrant' , $stage->id) }} enctype="multipart/form-data">
                        @csrf
                        <div class="card-header pb-0">


                                <div class="d-flex align-items-center">
                                    <p class="mb-0">Affecter un stage à un encadrant</p>
                                    {{-- {{$stage}} --}}
                                    ...
                                    <div class="form-group">
                                        <label for="encadrant_id">Encadrant:</label>
                                            <select name="encadrant_id"  id="encadrant_id" class="form-control">
                                                <option value="0">---choix encadrant ---</option>
                                                        @foreach ($enseignants as $enseignant) 
                                                             <option value="{{$enseignant->id}}" {{ old('enseignant_id',$enseignant->id ) == $encadrant_responsable?->id ? 'selected' : '' }}> {{$enseignant->user->nom }} {{ $enseignant->user->prenom }}  </option>
                                                        @endforeach
                                            </select>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="co_encadrant_id">Co-Encadrant:</label>
                                            <select name="co_encadrant_id"  id="co_encadrant_id" class="form-control">
                                                <option value="0">---choix co-encadrant ---</option>
                                                        @foreach ($enseignants as $enseignant) 
                                                             <option value="{{$enseignant->id}}" {{ old('enseignant_id',$enseignant->id ) == $co_encadrant_responsable?->id ? 'selected' : '' }}> {{$enseignant->user->nom }} {{ $enseignant->user->prenom }}  </option>
                                                        @endforeach
                                            </select>
                                    </div>
                                </div>
